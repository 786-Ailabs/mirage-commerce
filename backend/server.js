...
app.get("/api/health", async (_req, res) => {
  ...
  res.json({ ok: true, service: "N Mart Grocery API", mode: "local-json-db", counts: { products: db.products.length, orders: db.orders.length, customers: (db.customers || []).length, banners: db.settings?.banners?.length || 0 } });
});
...
app.listen(port, () => console.log(`N Mart backend running on http://127.0.0.1:${port}`));
