...
function printOrder(order) {
  ...
  </style></head><body><main class="page"><section class="head"><div class="brand"><h1>Mirage</h1><p>Digital Grocery Store</p><p>Fresh grocery delivery and local fulfilment</p></div><div class="invoice-box"><strong>Invoice / Order</strong><p>${order.id}</p><p class="muted">${formatDate(order.createdAt)}</p><p>Status: <strong>${order.status}</strong></p></div></section><section class="grid"><div class="box"><h3>Customer</h3><p><strong>${order.customer?.name || "Customer"}</strong></p><p>${order.customer?.phone || ""}</p><p>${order.customer?.address || ""}</p></div><div class="box"><h3>Delivery</h3><p><strong>${order.customer?.deliverySlot || "Slot not selected"}</strong></p><p>${order.customer?.note || "No special note"}</p></div></section><table><thead><tr><th>#</th><th>Item</th><th>Qty</th><th>Rate</th><th>Amount</th></tr></thead><tbody>${rows}</tbody></table><section class="totals"><div><span>Subtotal</span><strong>INR ${subtotal}</strong></div><div><span>Delivery</span><strong>${delivery ? `INR ${delivery}` : "Free"}</strong></div><div class="grand"><span>Total</span><strong>INR ${order.total}</strong></div></section><p class="footer">Thank you for shopping with Mirage. Please verify items during delivery. Perishable goods are packed fresh and should be stored immediately.</p><div class="actions"><button onclick="window.print()">Print / Save PDF</button></div></main></body></html>`;
...
export default function Admin({ products = [], settings = {}, customers = [], stats = {}, onAddProduct, onUpdateProduct, onDeleteProduct, onUploadProductImage, onUploadBanner, orders = [], adminNotice, onDismissNotice, onUpdateOrder }) {
  ...
  const [bannerTitle, setBannerTitle] = useState(settings?.activeBanner?.title || "Mirage fresh grocery store");
