...
function printOrder(order) {
  ...
  </style></head><body><main class="page"><section class="head"><div class="brand"><h1>N Mart</h1><p>Digital Grocery Store</p><p>Fresh grocery delivery and local fulfilment</p></div><div class="invoice-box"><strong>Invoice / Order</strong><p>${order.id}</p><p class="muted">${formatDate(order.createdAt)}</p><p>Status: <strong>${order.status}</strong></p></div></section><section class="grid"><div class="box"><h3>Customer</h3><p><strong>${order.customer?.name || "Customer"}</strong></p><p>${order.customer?.phone || ""}</p><p>${order.customer?.address || ""}</p></div><div class="box"><h3>Delivery</h3><p><strong>${order.customer?.deliverySlot || "Slot not selected"}</strong></p><p>${order.customer?.note || "No special note"}</p></div></section><table><thead><tr><th>#</th><th>Item</th><th>Qty</th><th>Rate</th><th>Amount</th></tr></thead><tbody>${rows}</tbody></table><section class="totals"><div><span>Subtotal</span><strong>INR ${subtotal}</strong></div><div><span>Delivery</span><strong>${delivery ? `INR ${delivery}` : "Free"}</strong></div><div class="grand"><span>Total</span><strong>INR ${order.total}</strong></div></section><p class="footer">Thank you for shopping with N Mart. Please verify items during delivery. Perishable goods are packed fresh and should be stored immediately.</p><div class="actions"><button onclick="window.print()">Print / Save PDF</button></div></main></body></html>`;
...
export default function Admin({ products = [], settings = {}, customers = [], stats = {}, onAddProduct, onUpdateProduct, onDeleteProduct, onUploadProductImage, onUploadBanner, orders = [], adminNotice, onDismissNotice, onUpdateOrder }) {
  ...
  const [bannerTitle, setBannerTitle] = useState(settings?.activeBanner?.title || "N Mart fresh grocery store");
  const [posterTitle, setPosterTitle] = useState("N Mart grocery poster");
  const [offerTitle, setOfferTitle] = useState("N Mart offer banner");
  ...
  async function handlePosterUpload(event) {
    ...
    setUploading("poster");
    try {
      for (const file of files) await onUploadBanner(file, posterTitle, "poster");
    }
    finally { setUploading(""); event.target.value = ""; }
  }
  ...
  async function handleOfferUpload(event) {
    ...
    setUploading("offer");
    try {
      for (const file of files) await onUploadBanner(file, offerTitle, "offer");
    }
    finally { setUploading(""); event.target.value = ""; }
  }
  ...
  return (
    <main className="admin-layout">
      ...
      <section className="admin-card marketing-upload-grid">
        <div className="marketing-upload-panel"><div><span className="eyebrow">Product posters</span><h2>Scroller posters</h2><p className="helper-text">These posters appear after every 6 products. Recommended size: 1600 x 450 px. Mobile safe area: keep important text in the centre 1200 x 340 px.</p></div><div className="banner-upload-row"><input placeholder="Poster title" value={posterTitle} onChange={(event) => setPosterTitle(event.target.value)} /><label className="upload-button">{uploading === "poster" ? "Uploading..." : "Upload Posters"}<input type="file" accept="image/*" multiple onChange={handlePosterUpload} /></label></div><div className="banner-thumb-row">{(settings?.posters || []).slice(0, 12).map((poster, index) => <figure key={poster.id || poster.imagePath} className="banner-preview"><img src={assetUrl(poster.imagePath)} alt={poster.title || `Poster ${index + 1}`} /><figcaption>Poster {index + 1}</figcaption></figure>)}</div></div>
        <div className="marketing-upload-panel"><div><span className="eyebrow">Offer banners</span><h2>Offer banner images</h2><p className="helper-text">These show inside the top offers section. Recommended size: 1200 x 375 px. Mobile safe area: keep important text in the centre 900 x 280 px.</p></div><div className="banner-upload-row"><input placeholder="Offer banner title" value={offerTitle} onChange={(event) => setOfferTitle(event.target.value)} /><label className="upload-button">{uploading === "offer" ? "Uploading..." : "Upload Offer Banners"}<input type="file" accept="image/*" multiple onChange={handleOfferUpload} /></label></div><div className="banner-thumb-row">{(settings?.offerBanners || []).slice(0, 8).map((offer, index) => <figure key={offer.id || offer.imagePath} className="banner-preview"><img src={assetUrl(offer.imagePath)} alt={offer.title || `Offer banner ${index + 1}`} /><figcaption>Offer {index + 1}</figcaption></figure>)}</div></div>
      </section>
