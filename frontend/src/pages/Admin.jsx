  ...
  return (
    <main className="admin-layout">
      ...
      <section className="admin-card marketing-upload-grid">
        <div className="marketing-upload-panel"><div><span className="eyebrow">Product posters</span><h2>Scroller posters</h2><p className="helper-text">These posters appear after every 6 products. Upload wide JPG/PNG/WebP images.</p></div><div className="banner-upload-row"><input placeholder="Poster title" value={posterTitle} onChange={(event) => setPosterTitle(event.target.value)} /><label className="upload-button">{uploading === "poster" ? "Uploading..." : "Upload Posters"}<input type="file" accept="image/*" multiple onChange={handlePosterUpload} /></label></div><div className="banner-thumb-row">{(settings?.posters || []).slice(0, 12).map((poster, index) => <figure key={poster.id || poster.imagePath} className="banner-preview"><img src={assetUrl(poster.imagePath)} alt={poster.title || `Poster ${index + 1}`} /><figcaption>Poster {index + 1}</figcaption></figure>)}</div></div>
        <div className="marketing-upload-panel"><div><span className="eyebrow">Offer banners</span><h2>Offer banner images</h2><p className="helper-text">These show inside the top offers section as separate promotional banners.</p></div><div className="banner-upload-row"><input placeholder="Offer banner title" value={offerTitle} onChange={(event) => setOfferTitle(event.target.value)} /><label className="upload-button">{uploading === "offer" ? "Uploading..." : "Upload Offer Banners"}<input type="file" accept="image/*" multiple onChange={handleOfferUpload} /></label></div><div className="banner-thumb-row">{(settings?.offerBanners || []).slice(0, 8).map((offer, index) => <figure key={offer.id || offer.imagePath} className="banner-preview"><img src={assetUrl(offer.imagePath)} alt={offer.title || `Offer banner ${index + 1}`} /><figcaption>Offer {index + 1}</figcaption></figure>)}</div></div>
      </section>
